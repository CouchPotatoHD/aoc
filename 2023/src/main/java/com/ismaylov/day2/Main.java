...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String> inputList = List.of("Game 1: 4 blue, 4 red, 16 green; 14 green, 5 red; 1 blue, 3 red, 5 green",
        ...
        Integer result = inputList
                .stream()
                .map(solving::parseGame)
                .filter(game -> solving.possibleGame(colourIntegerMap, game))
                .mapToInt(game -> game.number)
                .sum();

        System.out.println(result);

        Long result2 = inputList
                .stream()
                .map(solving::parseGame)
                .map(solving::possibleGame2)
                .reduce(0L, Long::sum);
